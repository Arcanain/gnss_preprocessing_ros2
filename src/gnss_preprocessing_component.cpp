...
#include <cmath>
#include "gnss_preprocessing/gnss_preprocessing_component.hpp"


int main(int argc, char **argv)
{
    rclcpp::init(argc, argv);
    auto node = std::make_shared<GnssPreprocessingComponent>(36.082862399999996, 140.0769906, 0.0); // 適切なlat, lon, higを指定
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
...
: Node("gnss_preprocessing_component"), lat0(lat), lon0(lon), hig0(hig)
{
    RCLCPP_INFO(this->get_logger(), "GnssPreprocessingComponent Start!");
...
{
    RCLCPP_INFO(this->get_logger(), "GnssPreprocessingComponent Finish");
}

void GnssPreprocessingComponent::gnssCallback(const sensor_msgs::msg::NavSatFix::SharedPtr gnss_msg)
{
    RCLCPP_INFO(this->get_logger(), "Received GNSS data: lat=%f, lon=%f, alt=%f", gnss_msg->latitude, gnss_msg->longitude, gnss_msg->altitude);
